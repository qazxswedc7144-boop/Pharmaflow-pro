...
export function SubscriptionOnboardingModal({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[1000] flex items-center justify-center p-2" dir="rtl">
        <motion.div 
          initial={{ opacity: 0, scale: 0.95, y: 20 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0.95, y: 20 }}
          transition={{ type: "spring", duration: 0.5 }}
          className="bg-white dark:bg-gray-900 rounded-[32px] border border-slate-100 dark:border-gray-800 shadow-2xl max-w-xs w-full overflow-hidden text-right p-2 relative"
        >
          {/* Decorative Sparkle Background */}
          <div className="absolute top-2 right-0 w-32 h-32 bg-emerald-500/5 rounded-full blur-3xl pointer-events-none" />
          <div className="absolute bottom-0 left-0 w-32 h-32 bg-blue-500/5 rounded-full blur-3xl pointer-events-none" />

          {/* Close button */}
          <button 
            onClick={onClose} 
            className="absolute top-3 left-3 w-7 h-7 flex items-center justify-center bg-emerald-50 dark:bg-emerald-950/40 hover:bg-emerald-100 dark:hover:bg-emerald-900/60 text-emerald-600 dark:text-emerald-400 border border-emerald-200/50 dark:border-emerald-800/40 rounded-full transition-all"
          >
            <X size={18} />
          </button>
          ...
          <div className="flex items-center gap-2 mb-1">
            ...
            <div>
              <h2 className="text-base font-black text-slate-900 dark:text-white">مرحباً بك في PharmaFlow Pro</h2>
          ...
          <div className="space-y-2 mb-1">
            ...
            <div className="grid grid-cols-2 gap-2 mt-1">
              <div className="bg-slate-50 dark:bg-gray-800/50 p-2 rounded-2xl border border-slate-100 dark:border-gray-850">
          ...
          <button
            onClick={onClose}
            className="w-full bg-[#1E4D4D] text-white py-1.5 rounded-2xl font-black text-sm hover:skew-x-1 transition-all shadow-xl shadow-slate-200 dark:shadow-none flex items-center justify-center gap-2 cursor-pointer animate-pulse"
          >
            <span>ابدأ التجربة المجانية الآن</span>
            <ArrowUpRight size={16} />
          </button>

          <SubscriptionContactFooter supportNumber="772093714" systemVersion="1.0.0" />
        </motion.div>
      </div>
    </AnimatePresence>
  );
...
export function SubscriptionGlobalUsageRibbon({ onUpgrade }: { onUpgrade: () => void }) {
  ...
  return (
    <div className="w-full bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800/60 px-6 py-1.5">
      <div className="max-w-xs mx-auto flex flex-col sm:flex-row items-center justify-between gap-2 text-right">
        {/* Metric Label */}
        <div className="flex items-center gap-2">
          <div className={`text-[10px] font-black uppercase px-2.5 py-1.5 rounded-full border ${ribbonColors} flex items-center gap-2`}>
        ...
        <button 
          onClick={onUpgrade}
          className="flex items-center gap-2 text-[11px] font-black bg-blue-600 hover:bg-blue-700 text-white px-4 py-1.5.5 rounded-xl transition-all shadow-xs cursor-pointer"
        >
...
export function SubscriptionWarningInterceptor() {
  ...
  return (
    <AnimatePresence>
      <div className="fixed bottom-6 left-6 z-[900] max-w-xs w-full p-2" dir="rtl">
        <motion.div 
          initial={{ opacity: 0, y: 50, scale: 0.95 }}
          animate={{ opacity: 1, y: 0, scale: 1 }}
          exit={{ opacity: 0, y: 50, scale: 0.95 }}
          className="bg-white dark:bg-gray-900 rounded-2xl border-2 border-amber-400 p-2 shadow-2xl text-right flex flex-col gap-2 relative overflow-hidden"
        >
          {/* Subtle colored warning border glow */}
          <div className="absolute top-2 right-0 w-3 h-full bg-amber-400" />

          <button 
            onClick={() => setHasDismissed(true)}
            className="absolute top-2 left-4 text-slate-400 hover:text-slate-600"
          >
            <X size={15} />
          </button>
          ...
          <div className="flex gap-2 justify-end mt-1">
            <button 
              ...
              className="bg-amber-500 hover:bg-amber-600 text-white text-[10px] font-black px-4 py-1.5 rounded-lg transition-all cursor-pointer"
            >
              ترقية الآن
            </button>
            <button 
              onClick={() => setHasDismissed(true)} 
              className="bg-slate-100 dark:bg-gray-800 text-slate-500 dark:text-gray-300 text-[10px] font-bold px-3 py-1.5 rounded-lg transition-all cursor-pointer"
            >
              لاحقاً
            </button>
          </div>
...
export function SubscriptionBlockadeBackdrop({ onUpgrade }: { onUpgrade: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[2000] flex items-center justify-center p-2 overflow-y-auto" dir="rtl">
      <motion.div 
        initial={{ opacity: 0, scale: 0.9, y: 30 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        transition={{ type: "spring", duration: 0.5 }}
        className="bg-white dark:bg-gray-900 rounded-[32px] border border-slate-100 dark:border-slate-800 shadow-2xl max-w-xs w-full text-right p-2 relative overflow-hidden"
      >
        ...
        <h3 className="text-base font-black text-rose-600 text-center mb-1">انتهت النسخة التجريبية</h3>
        <p className="text-slate-500 dark:text-gray-400 text-xs font-bold text-center mb-1">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-1">
          ...
          <div className="bg-emerald-50/50 dark:bg-emerald-950/10 border border-emerald-100/50 dark:border-emerald-900/30 rounded-2xl p-2">
            <span className="text-[11px] font-black text-emerald-600 block mb-1 font-black">العمليات المسموح بها:</span>
          ...
          <div className="bg-rose-50/50 dark:bg-rose-950/10 border border-rose-100/50 dark:border-rose-900/30 rounded-2xl p-2">
            <span className="text-[11px] font-black text-rose-600 block mb-1 font-black">العمليات المحظورة (مغلقة):</span>
            <ul className="space-y-2 text-xs font-bold text-slate-600 dark:text-gray-300">
              <li className="flex items-center gap-2">
        ...
        <div className="space-y-2">
          <button 
            ...
            className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black py-1.5 rounded-2xl text-xs flex items-center justify-center gap-2 shadow-lg shadow-blue-500/20 cursor-pointer"
          >
            <CreditCard size={16} />
...
export function ReviewerSaaSTester() {
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-900 border border-slate-150 dark:border-slate-800 p-2 rounded-2xl text-right mt-1">
      ...
      <p className="text-[11px] text-slate-500 dark:text-slate-400 mb-1 leading-relaxed font-bold">
      ...
      <div className="space-y-2">
        ...
        {/* Plan Selector */}
        <div>
          <span className="text-[11px] text-slate-600 dark:text-slate-300 font-extrabold block mb-1">استبدال الخطة ترخيصاً:</span>
          <div className="grid grid-cols-4 gap-2">
            {[
              ...
              <button
                ...
                className={`py-1.5 rounded-xl text-center text-[10px] font-black transition-all ${currentPlan === item.code ? 'bg-[#1E4D4D] text-white' : 'bg-white dark:bg-gray-800 text-slate-600 dark:text-gray-300 border border-slate-100 dark:border-gray-700 hover:bg-slate-50'}`}
              >
                {item.label}
              </button>
            ))}
        ...
        {/* Shortcuts */}
        <div className="flex justify-between items-center bg-white dark:bg-slate-800/80 p-2 rounded-xl border border-slate-100 dark:border-gray-750">
          ...
          <div className="flex gap-2">
            <button 
              ...
              className="px-2 py-1.5 text-[9px] font-bold text-amber-700 bg-amber-50 dark:bg-amber-950/20 hover:bg-amber-100 rounded border border-amber-200"
            >
              شغّل تحذير (182)
            </button>
            <button 
              ...
              className="px-2 py-1.5 text-[9px] font-bold text-rose-700 bg-rose-50 dark:bg-rose-950/20 hover:bg-rose-100 rounded border border-rose-200"
            >
              شغّل كتلة (203)
            </button>
            <button 
              ...
              className="px-2 py-1.5 text-[9px] font-bold text-slate-600 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 rounded border border-slate-200"
            >
              إعادة تصفير
            </button>
          </div>
...
export function TrialBlockedModal({ onClose }: { onClose: () => void }) {
  return (
    <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-[3000] flex items-center justify-center p-2 text-center" dir="rtl">
      <motion.div
        initial={{ opacity: 0, scale: 0.95, y: 10 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        className="bg-white dark:bg-gray-900 rounded-[32px] border border-slate-100 dark:border-gray-800 shadow-2xl max-w-xs w-full overflow-hidden p-2 relative"
      >
        <div className="flex justify-center mb-1">
          <div className="w-16 h-16 bg-rose-50 dark:bg-rose-950/45 text-rose-500 rounded-full flex items-center justify-center animate-bounce">
            <ShieldAlert size={36} />
        ...
        <h3 className="text-base font-black text-rose-600 mb-1">انتهت النسخة التجريبية</h3>
        <p className="text-slate-700 dark:text-gray-300 text-sm font-bold leading-relaxed mb-1">
        ...
        <div className="space-y-2">
          ...
          <button
            onClick={onClose}
            className="w-full bg-slate-50 hover:bg-slate-100 dark:bg-gray-800 dark:hover:bg-gray-750 text-slate-500 dark:text-gray-300 font-black py-1.5 rounded-2xl text-xs cursor-pointer"
          >
            إغلاق النافذة
          </button>
        </div>
